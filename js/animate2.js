...
function animate (ele,obj,duration,effect,callback) {
    ...
    for(var attr in obj) {
        ...
        var distance = target - begin ;
        if(distance) {
            oBegin[attr] = begin;
            oDistance[attr] = distance;
            flag++;
        }
    }
    if(flag === 0) {
    ...
    function step () {
        times += interval;
        if(times < duration) {
            for(var attr in oDistance) {
                var begin = oBegin[attr];
                var distance = oDistance[attr];
                var val = fnEffect(times,begin,distance,duration);
                setCss(ele,attr,val);
            }
        } else {
            for(var attr in oDistance) {
...
var zhufengEffect = {
    ...
    Expo: {//指数曲线的缓动（2^t）；
        ...
        easeOut: function(t,b,c,d){//var val=fnEffect(times,begin,change,duration);
